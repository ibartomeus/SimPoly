...
#' @param white_noiseCV Numeric, the expected CV across years
#'
#' @return A data.frame.
#' @export
#'
...
true_abundance <- function(n_round = 8, startmonth = 2, endmonth = 10,
  ...
  for(j in 1:n_years){
    ...
    sd = pars$h*white_noiseCV #mean * CV, correct?
    pars$h2 <- pars$h
    for(k in 1:length(pars$h)){
      pars$h2[k] <- rnorm(1, pars$h[k], sd[k]) #this is very little white noise. Revisit once data is available
    }
    #plot(pars$h2, pars$h)
